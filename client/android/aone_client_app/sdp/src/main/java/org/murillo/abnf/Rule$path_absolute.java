...
final public class Rule$path_absolute extends Rule
{
  private Rule$path_absolute(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$path_absolute parse(ParserContext context)
  {
    ...
    rule = null;
    if (parsed)
      rule = new Rule$path_absolute(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("path-absolute", parsed);

    return (Rule$path_absolute)rule;
  }
}
