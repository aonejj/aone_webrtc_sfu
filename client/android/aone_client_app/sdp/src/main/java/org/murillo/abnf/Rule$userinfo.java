...
final public class Rule$userinfo extends Rule
{
  private Rule$userinfo(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$userinfo parse(ParserContext context)
  {
    ...
    rule = null;
    if (parsed)
      rule = new Rule$userinfo(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("userinfo", parsed);

    return (Rule$userinfo)rule;
  }
}
