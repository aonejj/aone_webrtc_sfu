...
final public class Rule$URI extends Rule
{
  private Rule$URI(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$URI parse(ParserContext context)
  {
    context.push("URI");
    ...
    rule = null;
    if (parsed)
      rule = new Rule$URI(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("URI", parsed);

    return (Rule$URI)rule;
  }
}
