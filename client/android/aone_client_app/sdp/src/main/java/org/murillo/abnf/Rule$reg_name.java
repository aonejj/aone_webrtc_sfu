...
final public class Rule$reg_name extends Rule
{
  private Rule$reg_name(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$reg_name parse(ParserContext context)
  {
    ...
    rule = null;
    if (parsed)
      rule = new Rule$reg_name(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("reg-name", parsed);

    return (Rule$reg_name)rule;
  }
}
