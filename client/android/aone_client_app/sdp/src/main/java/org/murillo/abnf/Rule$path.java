...
final public class Rule$path extends Rule
{
  private Rule$path(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$path parse(ParserContext context)
  {
    context.push("path");
    ...
    rule = null;
    if (parsed)
      rule = new Rule$path(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("path", parsed);
