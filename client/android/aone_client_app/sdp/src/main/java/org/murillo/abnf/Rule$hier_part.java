...
final public class Rule$hier_part extends Rule
{
  private Rule$hier_part(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$hier_part parse(ParserContext context)
  {
    ...
    rule = null;
    if (parsed)
      rule = new Rule$hier_part(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("hier-part", parsed);

    return (Rule$hier_part)rule;
  }
}
