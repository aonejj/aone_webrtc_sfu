...
final public class Rule$FWS extends Rule
{
  private Rule$FWS(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$FWS parse(ParserContext context)
  {
    context.push("FWS");
    ...
    {
      {
        ...
        if (parsed)
        {
          ...
          for (int i1 = 0; i1 < 1 && f1; i1++)
          {
            rule = Rule$obs_FWS.parse(context);
            if ((f1 = rule != null))
            {
              e1.add(rule);
              c1++;
    ...
    rule = null;
    if (parsed)
      rule = new Rule$FWS(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("FWS", parsed);

    return (Rule$FWS)rule;
  }
}
